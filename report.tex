...
Nous nous appuyeront sur cette grammaire, que nous appelerons grammaire intiale, que nous convertirons en format ANTLR et ferons les modifications jugés nécessaire.

Nous fairons attention à son extensibilitéset expliquerons des choix d'implémentation.

\subsection{Rappel}
Pour commencer, il est nécessaire de préciser ce que permet cette grammaire initiale.
On peut définir dans un fichier deux structures différentes : une collection et un schéma.

\textbf{Collection :}
Une collection est un mapping ou une séquence dans un format proche du JSON ou en YAML.
Les collections peuvent être imbriqués.
Mapping :
Définit des clé-valeur.
Une clé peut posséder des attribut de présentation (ex name( optional : true) : name);
Une valeur peut être une collection ou un scalaire
Un scalaire est une valeur\dots

Un schéma est un document qui décrit la spécification d'un nouveau type.

Nous allons garder plus au moins cette strucure qui regroupe des éléments importants du langage UON.


\subsection{Modification}
En résumé ...
J'ai fait en sorte que les clé peuvents avoir des guillemets
Jai rajouter ke fait que des types peut avoir des propriété, c'était manquant.
J'ai rajouter des unités
Chiffres + scientifiques
json5
parler des grammaires ANTLR

Pour commencer, la grammaire initiale était conçu pour avoir deux notations différentes : une proche de json et l'autre de yaml.
...
% Il a été décidé au milieu du projet, de se focaliser d'abord sur le format proche de JSON, et de revenir sur cette aspect quand la prière version a été réalisé.
...
Un point contraignant, et que la grammaire est intrinsèquement liés à certaines fonctionnalités.
La coloration syntaxique doit refléter au mieux cette grammaire.
Mais les règles de construction sont différentes. Et l'on a envie parfois d'avoir des comportements spécifiques.


UON se veut ête moins ambigue que ces alternatives pour l'utilisateur.

Cette grammaire se trouve dans le fichier UON.g4. Nous allons mentionner les points les plus importants
...
%les points à considérer lorsqu'on passe d'une syntaxe à une autre

Il a fallu réadapté la grammaire dans un autre langage. Heursement ANTLR et Lark se base sur une notation EBNF.
Ce qui fait que la strucure reste semblable.
Le seul point de divergence, à été l'utilisation de regex.
Des règles changes mais de toute façon, certaines on été changé...

\subsection{Implémentation}
Nous allons parcourir ensemble les points les plus importants de la grammaire etc\dots
...
Dans un document on peut définir une collection d'objet (un mapping ou un séquence) ou un schéma.
Avoir une sépération permet d'éviter les ambuiguités.
1 fichier pour 1 role distincs

\subsection{Clé}
Une clé peut être annoté entre guillement ou non


\subsection{Debugging}

Il n'est pas simple de debugger une grammaire seulement à l'aide d'un terminal, cela peut même se revéler extremement difficile.
Heursement pour nous, il existe une \href{https://plugins.jetbrains.com/plugin/7358-antlr-v4}{extension} sur intellij qui nous permet de visulaiser en temps réel, le parse tree équivalent au texte que l'on saisie.
...
\subsection{Mots-clés reservés}
UON contient une liste de mot clé reservés
%\begin{array}{l | r}
%    Symbol & Description
%    \hline
%    :      & Key value separator
