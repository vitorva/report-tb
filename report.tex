...
ANTLR a un mécanisme pour signaler des erreurs et s'en rétablir lors du processus de parsing.
...
S'il ANTLR n'a pas pu se rétablir et qu'il a encore des erreurs, on va consommer tout les tokens qui suit celui qui a posé problème. Le mécanimse s'arretera quand il réussira à trouver
un token qui lui permettra de quitter la règle de parser courrant sans la valider mais permettant de continuer le processus de parsing.

C'est donc grâce à ce procédé que l'on obtiendra un parse tree même s'il contient des erreurs.
...
Il s'agit d'un identifiant.

C'est grâce au scope d'un token, qu'un fichier de thème peut attribuer une couleur avec une mécanisme de "clé-valeur".
...
Avec ce procédé cela nous permet de profiter des thèmes de VS Code.
...
Prenons comme exemple un exemple simple, le scope pour les nombres : "constant.numeric.uon"

On voit qu'il s'agit d'une suite d'identifiant.

La convention est d'avoir des mots clés séparé par un point à chaque pour spécifier un niveau de précision supplémentaire

Il est aussi possible d'avoir plusieurs scope pour un élément :

"string.uon support.type.property-name.uon"
...
La coloration syntaxique doit refléter au mieux cette grammaire. Il y a donc une coloration spécifique les propriétés appartenant à un type.
Cela rends la coloration syntaxique fortement liée à la grammaire mais on se rends compte d'un coup d'oeuil si la propriété et correcte.
