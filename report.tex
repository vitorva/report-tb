...
Les explications complémentaires concernant l'input à traiter et l'indice à fournir se trouve plus tard, à la section \ref{candidates}.
...
Pour comprendre certains choix fait par la suite, il est nécessaire de d'expliquer l'indexation.
...
Ce qui est fait actuellement c'est de récupérer ces tokens dans un canal mais sans le considérer dans le langage comme un token faisant parti intégrante dans une règle.
C'est ce qui est recommandé dans la documentation du projet c3 de ne pas ignorer les espaces. Car cela permet de gérer plus de cas particuliers.
% https://neuroning.com/post/implementing-code-completion-for-vscode-with-antlr/

Même si l'on n'en profite pas actuellment, cela ne modifie pas le code en soit et cela permet l'extensibilité du code. % TODO : à vérifier

% La documentation du projet indique bien que ce n'est pas un problème trivial à résoudre.

\subsubsection{Trouver l'indice}
Donc pour demander des candidats, il serait nécessaire de convertir la position du curseur dans un indice. Mais seulement si on prend en compte tout le code.

La méthode actuellement appliqué et qu'on ne va pas traiter le code entier de la page.
Seulement le code entre le début de la page et jusqu'à la position du curseur sera extrait.
Car dans le cadre de la grammaire utilisé dans ce projet, le code se trouvant après un élément après le curseur n'a pas d'impact.

Actuellement ce qu'on veut c'est le prochain token possible
...
Ainsi lorsque l'on passera cet input au token stream qui retournera la liste des tokens trouvé dans l'ordre dans le texte.
On pourra déduire l'indice du prochain candidats en se basant sur la taille du tableau.
On peut déduire l'indice du prochain token en se basant sur la taille du token stream % ref.
Ce token stream

En soustrayant la valeur de 3 .... on arrive à obtenir à chaque fois un candidats. % TODO
