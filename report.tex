...
Presque tout peut être customisé et amélioré à travers de \href{https://code.visualstudio.com/api/extension-capabilities/overview}{L'Extension API}.
...
VScode fournit plein de documentations détaillées sur plein de sujets, guides, ainsi que des exemples de codes.
...
\begin{itemize}
    ...
    \item \textbf{\href{https://code.visualstudio.com/api/references/contribution-points}{Contribution Points}}: Des déclarations statiques qui sont faites dans l'Extension Manifest "package.json" pour étendre l'extension. Il s'agit d'un ensemble de déclarations JSON faites au travers du champ "contributes".
...
La fonction activate est executée à l'activation de notre extension par un Activation Event. On définira ici le code à exécuter.
...
\textbf{Remarque} : Le niveau de customisation est assez élevé. La seule limitation souvent indiquée et qu'il n'est pas possible d'accéder au DOM de l'éditeur.
...
Comme ce rapport n'a pas pour vocation d'expliquer tout ce qu'il est possible de faire dans une extension. Si vous souhaitez vous renseigner d'avantages sur ce sujet, vous pouvez le faire \href{https://code.visualstudio.com/api}{ici}.

La même remarque concerne aussi les fonctionnalités générales de l'éditeur VScode. Pour plus de détail, consulter \href{https://code.visualstudio.com/docs}{la documentation officielle}.
...
Cependant dans le cadre de son utilisation dans un éditeur/IDE, il devrait être "fault tolerant". Le parseur doit pouvoir générer un AST (abstract syntax tree) qui à du sens depuis du code incomplet.
...
Plein d'outils permettant la génération existent à partir d'une grammaire. J'ai opté pour le générateur de parser d'ANTLR (ANother Tool for Language Recognition), car :
...
\textbf{Remarque} :
Comme mentionner au point \hyperref[grammar scope]{Scope de la grammaire}. Mon prédécesseur avait utilisé Lark pour écrire sa grammaire et générer son Parser pour son travail.
...
La perspective de disposer d'un protocole de communication applicatif à la fois interopérable et adapté aux échanges à faible puissance est un très appréciable.
...
Il faut ensuite ajouter la propriété "publisher" et saisir l'id de celui-ci dans le package.json.

Pour finalement il suffit dans un terminal les commandes suivantes :
\begin{lstlisting}[frame=single]
...
Il est ensuite facile en suite de manager l'extension au travers de la page web.
