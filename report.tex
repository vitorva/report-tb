...
\subsubsection*{Converter (outil permettant de convertir du code)}
\begin{itemize}
...
Le résultat de cette analyse est un \emph{parse Tree} (aussi nommé CST) qui représente toutes les dérivations concrètes de notre input en fonction de la grammaire.
...
Dans le cadre de ce projet, cet arbre sera utilisé lorsque cela sera nécessaire.
...
La communauté est aussi très active (stack overflow, gitter, issue github, etc.).
...
Le squelette de base d'une extension est illustré par la figure \ref{basic-structure}.
...
Concernant les fonctionnalités générales de l'éditeur VS Code, les informations se trouvent sur \href{https://code.visualstudio.com/docs}{la documentation officielle}.
...
On pourrait également se demander s'il est possible de se passer d'une telle structure et de n'utiliser à la place uniquement des expressions régulières.
Cependant le problème majeur avec cette approche et que les imbrications deviennent extrêmement difficiles à gérer. \cite{antlr-mega-tutorial}
...
Cette image a été générée en utilisant le plugin \href{https://marketplace.visualstudio.com/items?itemName=mike-lischke.vscode-antlr4}{ANTLR4 grammar syntax support}.
...
Par défaut c'est l'implémentation \emph{DefaultErrorStrategy} qui est appliquée et qui consiste à supprimer un token qui poserait problème ou en rajouter un s'il manquait, avant de
...
Lorsqu'on sérialise des données, elles se retrouvent sous forme binaire ou textuelle.
...
\subsection{Types}
Une caractéristique importante en UON est que toute valeur appartient à un type. Les types sont organisés en différentes catégories.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[Les types de bases en UON]{\label{type-base}Les types de bases en UON}
\end{figure}
...
\begin{itemize}
    \item \textbf{Embedded schema} : inclus dans le dataset, les informations sont mélangés avec les contraintes.
    ...
    \item \textbf{Separated schema} : n'est pas inclus dans le dataset, les informations et contraintes sont séparéss et non liées.
\end{itemize}

Chaque solution présente des avantages et des inconvénients.

\subsection{Liens}
Le dépôt du projet se trouve à l'adresse suivante : \url{https://github.com/uon-language/specification}
...
UON est par nature proche du format JSON. Mais UON étant "YAML compliant" à un certain degré (reduced set). Il est naturel de penser qu'un format YAML soit supporté, même si son utilisation est moins recommandée que son alternative.
...
Les terminaux traités par la grammaire couvrent certains types, des nombres, des strings et des unités (temperature, kilogram, etc...).

\section{Modification}\label{modification}

La structure de départ a été gardée, car cette grammaire regroupe déjà des éléments importants du langage UON.
...
La première est de rajouter des tokens de retour à la ligne ou il serait jugé possible de les avoir dans les règles propres au format JSON, et de les rendre optionnels.
...
\begin{listing}[H]
    ...
    IDENTIFIER
        ...
        | '!';
    \end{minted}
    \caption{Règle concernant les strings sans guillemet}
    \label{unquoted-rule}
\end{listing}
...
\begin{listing}[H]
    ...
    \end{minted}
    \caption{Règle concernant les strings avec guillemet}
    \label{quoted-rule}
\end{listing}

N'importe quelle caractère peut se trouver entre guillemet sauf un autre guillemet et pareillement pour les apostrophes.
...
les points suivants ont été rajoutés à la figure \ref{project-structure}

\textbf{Remarque} : Les "..." signifie qu'il y a des sous fichiers et/ou répertoire, mais ont été ignoré pour ne pas surcharger
la figure.
...
Car depuis mocha, il n'y a pas moyen de le faire.
...
Ce procédé nous permet de profiter des thèmes de VS Code implémenté par défaut dans l'éditeur. Par exemple, illustrées par les figures \ref{vscode-dark-theme} et \ref{vscode-kimbi-dark-theme}.
...
La section \emph{textmate scopes} affiche la liste des scopes utilisés pour le token qu'on sélectionne. Le scope le plus spécialisé se trouve au sommet.
...
La figure \ref{Textmate grammar} illustre une représentation très simplifiée du fichier \emph{uon.tmLanguage.json}.
...
Dans cet exemple, la coloration n'est appliquée que sur les commentaires et les clés sans guillemets du langage UON.
Tout n'est pas couvert dans cet exemple, mais cela permet de bien comprendre le mécanisme général utilisé.
...
C'est un tableau contenant les règles actuelles utilisées pour parser le document en sein du scope courant.
...
\textbf{"name"} : le nom d'un scope qui sera attribué.
...
On aura donc deux captures différentes, illustré par la figure \ref{capture}.
...
Mais cela permet de savoir qu'une propriété n'est pas valide (en complément avec les messages d'erreurs).
...
Ces suggestions sont des compléments qui pourraient convenir à la chaine de caractère saisie. Cela va donc dépendre principalement du langage traité.
...
Il n'aurait donc rien de base à proposer pour ces deux langages, sans utiliser une source d'information externe comme les "JSON-schéma".
...
C'est pourquoi lorsqu'il faut implémenter une telle fonctionnalité, il faut savoir quoi proposer et quand le faire.
...
Mais en complément, nous allons aussi utiliser \href{https://github.com/mike-lischke/antlr4-c3}{"antlr4-c3"} (également simplement nommé c3).
...
Les suggestions proposées dans l'éditeur seront majoritairement des candidats fournis par le moteur. C'est donc un outil relativement puissant, qui simplifie grandement la tâche du développeur.

Il a été conçu par \href{https://github.com/mike-lischke}{Mike Lischke}. Cet outil est écrit en Typescript et est disponible comme node module. Il existe aussi en version \href{https://github.com/mike-lischke/antlr4-c3/tree/master/ports/java}{JAVA} et \href{https://github.com/mike-lischke/antlr4-c3/tree/master/ports/c%23}{C\#}.

Le moteur de complétion n'utilise pas le parse tree pour proposer des candidats. Mais l'exécution du parser est nécessaire pour remplir le token stream. Pour rappel, un exemple d'utilisation du parser est illustré précédemment par la figure \ref{Utilisation du parser d'ANTLR}.
...
L'indexation est gérée de deux manières différentes comme illustrée par la figure \ref{candidat-index}, tirée de la documentation de c3.
...
Même si l'on n'en profite pas actuellement, cela pourrait permettre une meilleure extensibilité du code si nécessaire.
...
Mais cela est n'est pas réellement nécessaire, si on ne prend pas en compte tout le code.

La méthode actuellement appliquée et que l'on va extraire le code saisi jusqu'à la position du curseur.
C'est ce code qui sera l'input que l'on va donner au parser lorsque la complétion sera trigger par VS Code.

Il sera ensuite possible de déduire l'indice du prochain candidat en se basant sur le tableau de tokens que le token stream contient ainsi que sa taille.

Car on remarque que chaque indice à la figure \ref{candidat-index} représente l'emplacement d'un token et un espace (si gardé). Et c'est ce que l'on retrouve dans le token stream.
...
Le numéro 10 serait un indice valide.
...
Les tokens qui sont utiliser comme suggestions sont ceux issus de a première catégorie. Mais quelques tokens sont ignorés : comme les parenthèses et accolades, car ce n'est pas vraiment pertinent. % et les deux-points
...
Parce que l'on pourrait avoir dans le stream de token des éléments qui ne seront pas compris par le moteur et l'on n'obtiendra pas de candidats.

Une explication fournit par le créateur du moteur dans la rubrique "issues" du dépôt du \href{ https://github.com/mike-lischke/antlr4-c3/issues/29}{projet}.
...
Parce que si l'on modifie la grammaire, il faudra répercuter les changements dans l'application.
...
Ce modèle de snippet permet d'afficher une chaine de caractères avec des cases vides à remplir.
...
ces propriétés ce trouve dans les candidats du moteur et qu'il ne s'agit pas de schéma en contrôlant dans le stream de token que le token \emph{!schema} ne s'y trouve pas.
...
Il est possible d'ajouter au provider des symboles (triggerCharacters) qui vont forcer son activation. Les symboles \emph{espace} " " et de \emph{retour à la ligne} (\textbackslash rn) ont été définis comme triggers, car cela semble assez naturel dans ce langage.
...
qui doit être remplacé et s'il doit même le faire.
...
Cela implique de gérer plein de cas particuliers de la grammaire depuis l'application. Il n'a pas été jugé pertinent de traiter ce cas, car n'apportant pas réellement un intérêt.

\subsection{Mécanisme automatique}

Il est parfois difficile de savoir ce que VS Code fournit par défaut ou à quel point un comportement peut être modifié.
C'est le cas pour la complétion. Il est nécessaire d'aller regarder dans les paramètres (le fichier \emph{settings.json} dans VS Code) pour voir d'ou viennent certains comportements.
...
Voici une liste non exhaustive de complétions possibles.
...
Elle est actualisée en continu pour refléter les changements.
...
Cette séparation a été faite pour simplifier des étapes de constructions et de filtrage des éléments.
...
Tout simplement, car on aurait du le faire dans la fonction d'agrégation et l'on préféra ici déléguer la logique dans le noeud parent concerné
...
Un parent va pouvoir obtenir la liste des enfants dans un ordre que l'on s'attendrait à recevoir. Car ils apparaissent comme ils ont été définis dans les règles grâce au parcours DFS.
Cela rend la manipulation dans un parent plus simple concernant les modifications à effectuer pour représenter ces informations en un objet de type \emph{DocumentSymbol}.
...
L'implémentation des suggestions peut se faire à l'aide d'un quickfick qui va permettre à l'utilisateur d'interagir avec l'éditeur.

Un petit quick fix a été implémenté pour en tester la mise en place. Il permet de supprimer le token qui pose problème en se basant sur les ranges fourni lors de la création d'un objet Diagnostic depuis le listener.
...
Le listener se trouve au niveau du code de la outline. Étant donné que c'est une fonctionnalité qui est souvent sollicitée, il a été jugé interessant
...
\begin{itemize}
    \item Se renseigner davantage sur VS Code et ANTLR pour comprendre certains comportements.
...
Passons maintenant en détail les tâches réalisées en lien avec le cahier des charges:

\textbf{VSCode Extension} :
\begin{itemize}
    \item L'extension est publiée sur le marketplace.
    \item Des tests simples sont effectués lors d'un push sur les branches \emph{dev} ou \emph{main} et lors de la publication d'une release.
    \item Lors d'une release, l'extension et publiée automatiquement sur le markeplace.
\end{itemize}

\textbf{Grammaire} :
\begin{itemize}
    \item La grammaire actuelle permet de créer des mappings et des séquences de type en un format proche de JSON et YAML, ainsi qu'un schéma de validation.
    \item Gère un certain nombre de valeurs (strings, nombres et unités).
    \item Les clés des mappings et les types peuvent avoir des propriétés.
\end{itemize}

\textbf{Syntax highlighting} :
\begin{itemize}
    \item Une coloration syntaxique est appliquée aux éléments.
\end{itemize}

\textbf{Auto-complétion} :
\begin{itemize}
    \item Des suggestions sont proposées si elles peuvent être écrit à l'emplacement souhaité.
    \item Les suggestions proposées à l'utilisateur sont les tokens suivants: (Les types et leurs propriétés associées, ainsi que des unités).
    \item Les suggestions peuvent aussi être un ensemble d'éléments (snippets).
\end{itemize}

\textbf{Outline view} :
\begin{itemize}
    \item Les éléments sont affichés dans l'outline d'une manière similaire à une outline de JSON ou de YAML.
    \item Les propriétés des clés et des types sont affichées comme enfant de l'élément.
\end{itemize}

\textbf{Lint} :
\begin{itemize}
    \item Les erreurs de syntaxes sont affichées : elles indiquent si un élément est faux ou manquant.
...
ANTLR s'est donc avéré être une composante dominante du projet. De plus sa gestion des erreurs fournie par défaut permet d'avoir une meilleure tolérance concernant celles-ci.
...
\begin{itemize}
    \item Imbriquer des mappings ou sequences en format YAML ne fonctionne pas correctement. Cela vient de la gestion des indentations qu'il faudrait corriger.
    \item L'éditeur affiche un caractère erroné au début du document lorsque le fichier est vide. Cela vient aussi de la gestion des indentations.
\end{itemize}
