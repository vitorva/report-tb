...
Comme ce rapport n'a pas pour vocation d'expliquer tout ce qui est possible de faire dans une extension. Si vous souhaitez vous renseinger d'avantages sur ce sujet, vous pouvez le faire \href{https://code.visualstudio.com/api}{ici}.
...
Il semble normal que la complétion n'ait pas lieu si le code précédent est gravement erroné. Cependant il devrait être tolérent sur des erreurs qui ne péjore pas la comprhéension du code.
Par exemple dans notre cas, les erreurs sur les valeurs (par exemple une propriété "max : 12a") pourraient être omises en utilisant une grammaire qui nous permet d'avoir des types supportant le plus de symboles possibles.
Ainsi même si cela est faux syntaxiquement en UON, cela ne bloque pas le processus de l'auto-complétion.
Toutefois pour signaler à l'utilsiateur que la saisie est incorrect. On peut s'aider en parallele de la coloration syntaxique.
