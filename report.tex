...
Pour recevoir des candidats il suffit d'avoir un environnement comme présenté par la figure \ref{antlr-setup} d'installer le module npm \emph{antrl4ts} de rajouter les lignes suivantes.
...
Nous recevorns alors un objet \emph{CandidatesCollection}.
...
tokens : fait le lien entre le prochain token possible et une liste de tokens qui le suivent immédiatement, s'ils existent.

% TODO
Nous allons uniquement utiliser les clés de cette map pour proposer nos suggestions.
Concernant la liste il faut que les éléments qui le suivents soit obligatoire sans possiblité d'être omis.
Ce qui n'est jamais le cas dans notre grammaire. Nous n'avons jamais deux tokens de lexer qui se suivent.

rules : fait le lien entre la règle du parser actuelle et la liste de règles utilisé pour arriver à ce stade.
Ce map nous permet de questionner la liste des symboles pour retourner des informations supplémentaires.
Pour pouvoir exploiter ce mécanisme il est nécessaire d'implémenter une liste de symbole.
