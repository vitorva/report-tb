...
La figure \ref{antlr-setup} illustre un exemple simple d'initialisation.
...
La figure \ref{seq_ATN} montre un atn d'une règle de parser.
...
La grammaire qui a été adaptée dans le format d'ANTLR sera utilisé pour illustrer nos exemples.

Un fichier UON peut contenir trois structures différentes : les collections en json et yaml, ainsi qu'un schéma.
...
dans le langage. Cette structure a été reprise, mais expliquons dans la section \ref{modification}
les modifications nécessaires qui ont été apportées.

\textbf{Remarque} : par simplicité nous dirons simplement qu'il s'agit du format "json" ou "yaml".
...
Elle fourni du support de langage sur une grammaire fonctionnelle UON qui est capable de traiter des mappings et des séquences de type JSON et YAML, ainsi qu'un schéma de validation.
...
\begin{itemize}
    ...
    \item Une coloration syntaxique é été implémenté
    \item De l'auto-complétion  à été implémenté
    \item Une outline view a été implémenté
    \item Afficher des informations au survol d'une souris à été implémenté (trivial)
\end{itemize}
...
L'objectif de ce travail de bachelor était de pouvoir avoir à disposition dans l'éditeur, les éléments pour permettre aux mieux la rédaction d'un fichier UON.
Et cela, sans devoir consulter la documentation.
...
Heursement des outils existent pour en générer un depuis une grammaire.

faisant gagner un temps de développement énorme, en accélérant grandement la mise en place d'un support de langage.
...
De plus, comme c'est une implémentation en Typescript qui est utilisé, peu d'exemples existent et cela a compliqué le travail lorsque des erreurs survenaient.
...
Du temps a été passé pour essayer d'améliorer le comportement des erreurs sans succès. Le comportement par défaut n'étant jamais réelement améliorer.
...
Pour l'outline view. Il a fallu comprendre comment contruire l'objet.
Ajouter la gestion des propriétés s'est aussi revelé difficle.
...
Un changement peut avoir des répercussions sur l'ensemble de celles-ci.

C'est le premier "gros" projet que j'ai été ammené à réaliser seul.
...
Les messages d'erreurs sont ceux qu'ANTLR affiche en cas d'erreur et ne sont les plus "user-friendly". Il faudrait donc adapter ces messages.
Il faudrait aussi pouvoir proposer des corrections d'erreurs (ex : quickfix) plus avancées.

Le fichier \emph{UonCompletionErrorStrategy.ts} ne fait que surchager le comportement par défaut. Si le comportement doit être améliorer, c'est ce fichier qui doit être modifié.

\section{Erreur existante}

Imbriquer des mapping ou sequences en format YAML ne fonctionne pas correctement. Cela vient de la gestion des indentations qu'il faudrait corriger.
