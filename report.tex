...
On doit donner le "parse tree" créer par le parser à la fonction d'entrée \emph{visit} de notre visitor. % const ast = uonASTVisitor.visit(tree);

L'intéret d'utiliser le pattern Visitor est de pouvoir retourner un élément à chaque étape du parcours.

Le type de classe est générique. Et ce qui nous permet de manipuler un objet VS code.
...
Le résultat par défaut est une liste vide.

Pourquoi ne pas manipuler directement un object DocumentSymbol ? Tout simplement car
sinon on aurait du le faire dans la fonction d'aggrégation et l'on préfera ici déléguer la logique dans le noeud parent concerné
...
Pour rappel un parcours DFS et effectué de la manière affiché à la figure \ref*{Algorithme de parcours en profondeur (DFS)}
...
Ce qui est agréable dans ce parcours, c'est qu'un parent va pouvoir obtenir la liste des enfants dans un ordre que l'on s'attendrait à recevoir.
