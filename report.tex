...
La outline view est une représentation notre fichier, elle peut-être hiérarchique.

Un élément de l'outline représente un élément unique du texte ou une strucure.

Il est possible de l'obtenir en analysant manuellement le fichier texte (par exemple ligne par ligne). Mais cet approche n'est pas optimale er il serait diffile de représenter la strucutre hiérarchique du document.

Nous allons donc utiliser notre parser.
Le résulat du parsing est un "parse tree" qui contient beaucoup trop d'information supérflues.
C'est pourquoi il faut pouvoir le parcourir pour en tirer un AST. On va vouloir récupérer uniquement les informations que l'on juge pertinent pendant le parcours
L'outline view sera une représentation de notre AST.

Cependant on ne va pas créer d'abord un AST puis construire la outline view à l'aide de celui-ci.

Nous allors directement construire l'outline view à la volé pendant le parcours du parse tree.


C'est finalement le parcours
...
La première consiste à écouter \dots

La deuxième est l'apporche choisi. 
