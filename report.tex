...
Le moteur de complétion n'utlise pas le parse tree pour proposer ses suggestions mais on quand même besoin de lancer le parser pour remplir le stream de token (tokenStream) sur lequel le moteur s'appuyera.

Une conséquence à cela et qu'on ne peut pas profiter de la stratégie d'erreur d'ANTLR, car ce stream est généré avant la phase de création du parser tree comme illustré par la figure\ref*{antlr-setup}. Et c'est pendant la phase du parser que le processus de stratégie est appliqué.

%% TODO : ...
Mais on doit quand même donner le parser au moteur pour qu'il puisse se baser sur les ATN (machines à état) du parser.
