...
%% Modification apporté pour le support de langage (pas grand chose)
...
% La grammaire actuelle nous permet de définir des maps, séquences dans le format json et yaml.
% D'ajouter des propriétés à des types et également la possibilité de définir un schéma pour un type custom.
...
%TODO expliquer ici les règles et subtilités
...
Nous expliquons ici comment nous pourrions faire cohabiter ces 2 formats avec ANTLR. % nous avons fait
Puis il ne s'aggira que de reféter ces changements dans les fonctionnalités. % Et avons refléter ces changements...
...
Des propriétés de présentation %TODO : voir ....
...
% propriétés clé
% propriésé types
% key -> string-litteral expliquer pourquoi
% symbol
...
Mais peut être également généré depuis le contrôle panel à l'aide de la commande suivante : % TODO 

%TODO : grâce à cette réflexion on a par défaut plusieurs thèmes (ceux présents dans vscode).
% Montrer un thème supplémentaire
% Donc créer un thème pas forcément bien et contraignant...
...
% TODO : Quand afficher des erreurs
On affiche le texte en rouge quand un caractère non attendu est écrit.
% [^\\s,!] donc généralement cette règle se trouve à la fin de toutes les autres.
% Seul les espaces, virgule et ! peuvent encore être écrit sans "erreur"
...
%Une des difficultés observés lors de l'implémentation de la coloration et le couplage à faire avec la grammaire.
%On a pas envie de copier exactement la grammaire car ce qui nous intéresse ici est la coloration
%Et l'on a envie parfois d'avoir des comportements spécifiques.
...
Car on pourrait imagine proposer de la complétion basé sur les clés (même si vscode le ferait grâce au mécanisme par défaut). % TODO : à  moins d'avoir des schéma linked aussi ?
...
% Donc si on définit un nouveau
% Actuellement seulement un schéma de document ?
...
% TODO : exemple de fuzy search dans VS Code (comportement par défaut)

% abc suggestion
Par défaut la complétion proposé par VS Code porte sur les symboles déjà existants dans le code.

\subsection{Gestion des erreurs }\label{error handle} % TODO : déplacer les explciation sur le parser error recovery
...
Comme le moteur de complétion se base principalement sur le stream de token trouvé (tokenStream), on ne peut donc pas profiter autant de la stratégie d'erreur
d'ANTLR, car ce stream est généré avant la phase de création du parser comme illustré par la figure\ref*{antlr-setup}. Et c'est pendant la phase du parser que le processus de stratégie est appliqué.
Mais on doit quand même donner le parser au moteur parce que\dots
...
Créer une nouvelle classe de listener qui détecte les erreurs lors du parsing.
Stocker la position des tokens qui posent problème.
Modifier le texte récupérer dans l'éditeur localement en conséquence.
Relancer la phase de la création du parser en lui donnant le texte modifié.
...
%Malheursement des fois on n'a pas trop d'alternatives possibles
...
Cependant toute la difficulté et de proposer ces suggestions au bon moment, car ne faisant pas partie du moteur de complétions d'antlr.
Une idée et de se baser sur le dernier token reconnu. On pourrait ici savoir qu'il est le moment d'ajouter ce snippet, si le dernier token est de type "!schema".

\subsection{Séquence de token}

%Le moteur va récupérer la liste des tokens.
%Exemple d'un ligne de la grammaire et de ce qu'on récupre
%BOOL_TYPE = !bool
...
Mais on peut récupérer également une liste de tokens depuis le moteur.
...
Comme illustrer à la figure %TODO
...
Ces tokens doivent obligatoirement se suivre dans la grammaire % TODO : être de type lexer ?
et ne peuvent pas être omis.

Par exemple : % TODO : code
...
Pour l'instant, juste à titre de découverte sur comment implémenter un quickfix. %todo expliquer

Il y a un quickfix pour supprimer le token qui pose problème en se basant sur les ranges fourni lors de la création d'un objet Diagnostic dans le listener %TODO
...
%problème je n'arrive pas à afficher plusieurs erreurs
% avant un message en rouge dans la outline mais plus mnt :(
...
Travailler sur ce projet s'est revélé être passionnant.
