...
Si le temps le permet, toute la logique du code concernant les fonctionnalités riches (voir section \ref{Programmatic language features}) pourrait être déplacée dans un langage server.
...
\subsection{Types}
Une caractéristique importante en UON est que tout type d'information appartient à un type. Les types sont organisés en différentes catégories.
...
\begin{itemize}
    ...
    \item \textbf{Separated schema} : les informations et contraintes sont séparés et non liés.
\end{itemize}
...
On peut redéployer une extension qu'avec une version supérieure que la précédente.
...
Pour respecter au maximum les conventions attendues et s'assurer qu'une coloration automatique soit appliquée. La stratégie suivante a été adoptée : partir depuis un fichier de thème déjà créé (Thème par défaut de VS Code : Dark + ).
...
La figure \ref{Textmate grammar} illustre un exemple très simplifié du fichier \emph{uon.tmLanguage.json}.
...
% TODO : exemple concret, remplacer image
\begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{assets/figures/snippet-suggestion.png}
    \end{center}
...
\subsection{Triggers}
Il est possible d'ajouter au provider des symboles qui vont forcer son activation. Les symboles espace " " et de retour à la ligne (\\rn) ont été définis comme triggers, car cela semble assez naturel dans ce langage.
...
%Quand est-ce que ça trigger tout seul
%On peut modifier le comportement
% "editor.acceptSuggestionOnCommitCharacter": true in settings.json?

%On doit aller dans le ficheir settings.json
% on voit 3paramètre utiles :  Acceptsuggestion on commit caracter
% DocumentSelector
% https://github.com/Microsoft/vscode/issues/18372
% https://github.com/ChristianKohler/PathIntellisense/issues/52
% https://code.visualstudio.com/api/references/vscode-api#CompletionItem

L'éditeur va trigger automatiquement l'autocomplétions en appelant le provider lorsque l'on saisit plusieurs caractères.
...
% https://www.antlr.org/api/Java/org/antlr/v4/runtime/ANTLRErrorListener.html#syntaxError(org.antlr.v4.runtime.Recognizer,java.lang.Object,int,int,java.lang.String,org.antlr.v4.runtime.RecognitionException)
