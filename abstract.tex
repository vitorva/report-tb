...
Les premières semaines ont été majoritairement consacrées a se renseigner.
...
Le support de langage traitait initiallement uniquement une grammaire en UON qui permettait des construction dans un format proche en JSON.
avant d'ajouter le format YAML.
Les propriétés n'étaient pas aussi considéré dans un premier temps dans l'outline view. Puis elles ont été rajouté par la suite.

Les principales ressources utilisés ont été :

\begin{itemize}
    \item Le site de VS Code à été directement la source principale pour se renseigner sur la mise en place et la gestion d'une extensions.
    \item Des articles en ligne concernant ANTLR et le moteur de complétion.
    \item Et bien évidement stackoverflow pour tous types de questions.
\end{itemize}
